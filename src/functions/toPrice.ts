 ...
 * @param decimals 
 */
export const toPrice = (amount: bigint|number, decimals: number, symbol: string|undefined, locales?: Intl.LocalesArgument) => {
    ...
    if(symbol && isLegalCurrency(symbol, locales)) {
        ...
        return Intl.NumberFormat(locales, {style: "currency", currency: symbol}).format(Number(amount) / 10 ** decimals);
    }
    ...
    const integerPartWithThousandSeparator = 
        Intl.NumberFormat(locales, {style: "currency", currency: "USD"}).format(BigInt(integerPart))
        .replace(toPriceSymbol("USD", locales), "").trim()
