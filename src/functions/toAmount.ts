 ...
 * @param valueText
 */
export const toAmount = ( valueText: string ): { amount: bigint; decimals: number } => {
	let { amount, decimals } = _toAmount( valueText );
...
const _toAmount = ( valueText: string ): { amount: bigint; decimals: number } => {
	...
	if ( numbers.length === 1 ) {
		if ( valueText.startsWith( numbers[ 0 ] ) ) {
	...
	throw new Error( `[0EC6294F] Invalid input value. - valueText: ${ valueText }` );
};
